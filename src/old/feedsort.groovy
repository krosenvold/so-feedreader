...
saver.saveToAtom(feed, saveTo)


static void sortFeedReverse(SyndFeed feed) {
    // Sort in reverse chronological order by publishedDate
    // I don't understand the expression but it works.
    feed.entries.sort {a,b-> 
        (a.publishedDate != null 
            && b.publishedDate != null 
            && a.publishedDate.after(b.publishedDate)) ? 0 : 1 
    }
}
